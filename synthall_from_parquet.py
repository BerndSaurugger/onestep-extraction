...
def synth_images(parquet_file='groundtruth_parquets/sdv1_wb_groundtruth.parquet',steps=16,n_seeds=2,seed_offset=0,make_grid_every=0,outfolder='gen_synthall/',caption_offset=0,n_captions=100,model='runwayml/stable-diffusion-v1-5',download_parquets=True):
    ...
    for c in captions:
        ...
        for seed in range(seed_offset, seed_offset + n_seeds):
            ...
            fn = f'{outfolder}{outfolder_prompt}/{seed:04d}.jpg'
            os.makedirs(os.path.dirname(fn), exist_ok=True)  # sicherstellen, dass Pfad existiert
            image.save(fn)
            ...
            if make_grid_every and (seed + 1) % make_grid_every == 0:
                torchvision.utils.save_image(
                    torch.cat(imgs_out),
                    f'{outfolder}{outfolder_prompt}/grid_{outfolder_prompt}_{seed:04d}.jpg',
                    nrow=int(math.sqrt(make_grid_every)),
                    normalize=True
                )
                imgs_out = []
